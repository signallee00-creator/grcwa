...
def Epsilon_fft(dN, eps_grid, G, dtype_f=torch.float64, dtype_c=torch.complex128, device=None):
    ...
    if isinstance(eps_grid, (list, tuple)) and len(eps_grid) == 3 and eps_grid[0].ndim == 2:
        ...
        top = torch.vstack((epsx_fft, torch.zeros_like(epsx_fft)))
        bottom = torch.vstack((torch.zeros_like(epsx_fft), epsy_fft))
        eps2 = torch.hstack((top, bottom))
        ...
        top = torch.vstack((eps_fft, torch.zeros_like(eps_fft)))
        bottom = torch.vstack((torch.zeros_like(eps_fft), eps_fft))
        eps2 = torch.hstack((top, bottom))
    else:
...
def get_ifft_xline(x_coords, y0, s_in, G, dtype_c=torch.complex128, device=None):
    return get_ifft_xline_batch(x_coords, y0, s_in, G, dtype_c=dtype_c, device=device)


def get_ifft_yline(x0, y_coords, s_in, G, dtype_c=torch.complex128, device=None):
    return get_ifft_yline_batch(x0, y_coords, s_in, G, dtype_c=dtype_c, device=device)
