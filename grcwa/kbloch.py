...
def Lattice_Reciprocate(L1, L2, dtype=torch.float64, device=None):
    """Return reciprocal lattice vectors without the 2*pi factor."""
    ...
    Lk1 = torch.stack((L2[1] / det, -L2[0] / det))
    Lk2 = torch.stack((-L1[1] / det, L1[0] / det))
...
def Lattice_getG(nG, Lk1, Lk2, method=0):
    """
    Select reciprocal-lattice orders.

    The returned order indices satisfy:
        k = G[:, 0] * Lk1 + G[:, 1] * Lk2

    method = 0: circular truncation
    method = 1: parallelogramic truncation
    """
...
def Lattice_SetKs(G, kx0, ky0, Lk1, Lk2):
    """Construct kx, ky including all selected reciprocal orders."""
...
def Gsel_parallelogramic(nG, Lk1, Lk2):
    """Parallelogramic truncation, adapted from Liu's gsel.c."""
    dtype = Lk1.dtype if torch.is_tensor(Lk1) else torch.float64
...
def Gsel_circular(nG, Lk1, Lk2):
    """Circular truncation, adapted from Liu's gsel.c."""
    dtype = Lk1.dtype if torch.is_tensor(Lk1) else torch.float64
    device = Lk1.device if torch.is_tensor(Lk1) else None
    Lk1 = torch.as_tensor(Lk1, dtype=dtype, device=device)
    Lk2 = torch.as_tensor(Lk2, dtype=dtype, device=Lk1.device)
    ...
    scale = torch.sqrt(1.0 - uv ** 2 / (u * v) ** 2)
